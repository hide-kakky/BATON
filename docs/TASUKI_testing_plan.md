...
本ドキュメントは、TASUKI プロジェクトの品質保証（QA）およびテスト戦略をまとめたものです。
...
### 2.4 テスト環境と認証
- **ローカル開発環境 (推奨)**: `supabase start` で構築したローカル環境を使用。
- **Inbucket**: マジックリンクメールを捕捉し、API経由でログインリンクを取得。
- **Seedデータ**: `supabase/seed.sql` でテスト用ユーザーや初期データを投入。
